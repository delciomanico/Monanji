...
export default function ConsultCasesScreen() {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await apiService.searchCasesByBI(biNumber.trim());
      
      if (response.success) {
        // Transform API response to match expected format
        const transformedCases = response.data.cases.map((apiCase: any) => ({
          id: apiCase.id,
          caseNumber: apiCase.protocol_number,
          personName: apiCase.person_info?.name || 'Nome não informado',
          status: mapApiStatusToDisplay(apiCase.status),
          date: new Date(apiCase.created_at).toLocaleDateString('pt-BR'),
          relationship: mapApiRelationship(apiCase.relationship),
          personImage: 'https://images.unsplash.com/photo-1544725176-7c40e5a71c5e?w=150&h=150&fit=crop&crop=face',
          details: {
            fullName: apiCase.person_info?.name || 'Nome não informado',
            description: apiCase.person_info?.brief_description || 'Descrição não disponível',
            lastSeen: apiCase.created_at ? new Date(apiCase.created_at).toLocaleString('pt-BR') : '',
            location: apiCase.location || 'Local não informado',
            status: mapApiStatusToDisplay(apiCase.status),
            updates: [] // Will be populated from case history in future updates
          }
        }));
  ...
  const mapApiRelationship = (apiRelationship: string) => {
    const relationshipMap: { [key: string]: string } = {
      'reporter': 'Denunciante',
      'family_member': 'Familiar',
      'victim': 'Vítima'
    };
    return relationshipMap[apiRelationship] || 'Denunciante';
  };
